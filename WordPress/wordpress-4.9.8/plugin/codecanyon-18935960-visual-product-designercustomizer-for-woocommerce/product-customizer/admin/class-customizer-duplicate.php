    ...
    class WC_Admin_Duplicate_Customizer
    {
        ...
        public function show_clone_link($actions, $post)
        {
            ...
            $actions['duplicate'] = '<a href="' .
                wp_nonce_url(admin_url('edit.php?post_type=customizer&action=duplicate_customizer&amp;post=' .
                    $post->ID), 'woocommerce-duplicate-customizer_' . $post->ID) .
                '" title="' . __('Make a clone from this customizer', 'customizer')
                . '" rel="permalink">' . __('Clone', 'customizer') . '</a>';
        ...
        public function clone_button()
        {
            ...
            if (isset($_GET['post'])) {
                $notifyUrl = wp_nonce_url(admin_url("edit.php?post_type=customizer&action=duplicate_customizer&post=" .
                    absint($_GET['post'])), 'woocommerce-clone-customizer_' . $_GET['post']);
                ?>
                <div id="clone-action">
                    <a class="clone"
                       href="<?php echo esc_url($notifyUrl); ?>"><?php _e('Copy to a new draft', 'customizer'); ?></a>
                </div>
                <?php
            }
        }
        ...
        public function clone_customizer_action()
        {
            ...
            if (empty($_REQUEST['post'])) {
                wp_die(__('No customizers to duplicate has been supplied!', 'customizer'));
            }
            ...
            if (!empty($post)) {
                ...
                wp_die(__('Customizer clone creation failed, could not find original customizer:', 'customizer') . ' ' . $id);
            }
        }
