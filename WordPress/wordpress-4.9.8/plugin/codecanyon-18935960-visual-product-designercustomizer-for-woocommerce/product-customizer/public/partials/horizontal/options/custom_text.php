<?php
$colors = get_option('customizer_colors', array());
$fonts  = get_option('customizer_fonts', array());
if (empty($fonts)) {$fonts['Roboto'] = 'Roboto';}
?>

<div class="shop-filter__options-wrapper shop-filter_options_text shop-filter__options-wrapper--block"
     data-sf-group="options"
     data-sf-component-id="<?php echo $component['component_id'] ?>">
    <?php $count = 0; ?>
    <?php foreach ($component['options'] as $optionKey => $option): ?>
        <?php $count++; ?>
        <?php if ($option['option_enable'] == 1): ?>
            ...
            <div
                    class="shop-filter__option--block shop-filter__option <?php echo($count == count($component['options']) ? 'last' : ''); ?><?php echo ($option['option_default']) ? 'is-default' : ''; ?>"
                ...
                <div class="shop-filter__option__color-wrapper">
                    <div class="shop-filter__form-field-wrapper">
                        <input type="text" class="shop-filter__form-field shop-filter_custom_field"
                               placeholder="<?php echo __('Enter Your Text', 'customizer'); ?>"
                               onKeyUp="shopFilter.updateText(event,'<?php echo $option['option_id']; ?>', this.value)"
                    ...
                    <div class="shop-filter__option__color color_<?php echo $option['option_id']; ?>"
                         data-sf-elem="color" style="background-color: #<?php echo $firstColor; ?>"
                        ...
                        <div class="shop-filter__option__color-dropdown">
                            <?php foreach ($colors as $ckey => $cval): ?>
                                <div class="shop-filter__option__color-elem"
                                     title="<?php echo $cval; ?>"
                                     onClick="shopFilter.updateTextColor(event, '<?php echo $option['option_id']; ?>', '#<?php echo $ckey; ?>')"
                                     style="background-color: #<?php echo $ckey; ?>"></div>
                            <?php endforeach; ?>
                        </div>
                    </div>
                </div>
