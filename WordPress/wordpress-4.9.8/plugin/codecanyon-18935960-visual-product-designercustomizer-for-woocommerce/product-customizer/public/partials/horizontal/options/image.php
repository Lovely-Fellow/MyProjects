<div class="shop-filter__options-wrapper" data-sf-group="options"
	...
	<?php foreach ($component['groups'] as $group_title => $group): ?>
        <div class="customizer_group_name"><?php echo $group_title; ?></div>
		<?php foreach ($group as $optionKey => $option): ?>
			<?php if ($option['option_enable'] == 1): ?>
				...
				<?php $description = (empty($option['option_description']) ? esc_html($option['option_name']) . ($floatPrice ? ' (' . $price . ')' : '') : esc_html($option['option_description'])); ?>
            ...
            <?php if ($component_radio_button): ?>
                ...
                <div class="shop-filter__radio shop-filter__option-container tooltip-holder <?php echo !empty($option['option_default']) ? 'is-default' : ''; ?>"
                     data-sf-elem="option"
            ...
            <?php else: ?>
                <div
                        class="shop-filter__option-container tooltip-holder <?php echo ($option['option_default']) ? 'is-default' : ''; ?>"
                        data-sf-elem="option"
                        data-sf-data='{
                        "type" : "<?php echo Customizer_Public::COMPONENT_TYPE_IMAGE; ?>",
                        "option_id": "<?php echo $option['option_id']; ?>",
